...
public class RecoveryHeartFloorItem implements ObtainableItem {
    ...
    public RecoveryHeartFloorItem(Context context, DTile[][] tileSetPassed, Player passedPlayer, int startX, int startY){
    ...
    public Actor act(){
        ...
        try{
            ...
            errorText += "RecoveryHeartFloorItem.act ERROR 1 -" + e.toString();
        }
        return null;
    }
    ...
    public void onGet(PlayerStats playerStats){
        if(!gotMe) {
            playerStats.addItem(new RecoveryHeartHeldItem(playerStats));
            tickText = "-Picked up Recovery Heart";
            gotMe = true;
